...
def _resolve_upload_source_for_user(item: UploadSourceItem, user_id: str) -> dict[str, Any]:
    ...
    if upload_id:
        ...
        local_path = Path(Storage.get_file(file_item.path)).expanduser().resolve()
...
async def create_material_package_from_chat_upload(
    ...
    resolved_uploads = [_resolve_upload_source_for_user(item, str(user.id)) for item in upload_items]
...
async def get_generation_task_status(task_id: str, user: UserModel = Depends(get_verified_user)):
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        resp = await client.get(f'{base_url}/contents/generations/tasks/{safe_task_id}', headers=headers)

        if resp.status_code >= 400:
            # Compatibility fallback for providers that expose query-by-body API.
            fallback_resp = await client.post(
                f'{base_url}/contents/generations/tasks/query',
                headers=headers,
                json={'task_id': safe_task_id},
            )
